...
namespace common
{
    ...
    ErrorType SplineGenerator<N_DEG, N_DIM>::GetCubicSplineBySampleInterpolation(const vec_Vecf<N_DIM>& samples,
                                                                                 ...
                                                                                 SplineType* spline)
    {
            ...
            if (dis > para_delta_threshold)
            {
                ...
                for (int n = 0; n < n_inserted; n++)
                {
                    ...
                    if (para_insert < para[i])
                    {
                        ...
                        Eigen::Matrix<double,N_DIM,1> sample_insert;
        ...
        for (int i = 0; i < N_DIM; i++)
        {
            ...
            {
                Eigen::Matrix<double, N_DEG + 1, 1> coeff = Eigen::Matrix<double, N_DEG + 1, 1>::Zero();
    ...
    ErrorType SplineGenerator<N_DEG, N_DIM>::GetQuinticSplineBySampleFitting(
            const vec_Vecf<N_DIM>& samples, const std::vector<double>& para,
            const Eigen::ArrayXf& breaks, const double regulator,
            SplineType* spline)
    {
    ...
    ErrorType SplineGenerator<N_DEG, N_DIM>::GetSplineFromStateVec(
            const std::vector<double>& para, const vec_E<State>& state_vec,
            SplineType* spline) {
        if (N_DEG < 5) {
            printf(
    ...
    ErrorType SplineGenerator<N_DEG, N_DIM>::GetSplineFromFreeStateVec(
            const std::vector<double>& para, const vec_E<FreeState>& free_state_vec,
            SplineType* spline) {
        if (N_DEG < 5) {
            printf(
                    "[GetSplineFromStateVec]To fit state vec we need at least 5 "
    ...
    ErrorType SplineGenerator<N_DEG, N_DIM>::GetBezierSplineUsingCorridor(
            ...
            const vec_E<Eigen::Matrix<double,N_DIM,1>>& ref_points, const double& weight_proximity,
            BezierSplineType* bezier_spline)
    {
        ...
        {
            Eigen::Matrix<double, N_DEG + 1, N_DEG + 1> hessian = BezierUtils<N_DEG>::GetBezierHessianMat(derivative_degree);
            int idx, idy;
            double val;
            for (int n = 0; n < num_segments; n++)
            {
                double duration = cubes[n].t_ub - cubes[n].t_lb;
                for (int d = 0; d < N_DIM; d++)
                {
                    for (int j = 0; j < num_order; j++)
                    {
        ...
        if (!ref_stamps.empty())
        {
            ...
            for (int i = 0; i < num_ref_samples; i++)
            {
                ...
                double s = cubes[n].t_ub - cubes[n].t_lb;
                double t = ref_stamps[i] - cubes[n].t_lb;
                for (int d = 0; d < N_DIM; d++)
                {
                    for (int j = 0; j < num_order; j++)
                    {
                        ...
                        for (int k = 0; k < num_order; k++)
                        {
                            ...
                            nchoosek(N_DEG, k) * pow(t / s, j + k) * pow(1 - t / s, 2 * N_DEG - j - k);
                        }
                    }
                }
            }
        ...
        double val;
        {
            ...
            for (int n = 0; n < num_connections; n++)
            {
                double duration_l = cubes[n].t_ub - cubes[n].t_lb;
                double duration_r = cubes[n + 1].t_ub - cubes[n + 1].t_lb;
                for (int c = 0; c < num_continuity; c++)
                {
                    double scale_l = pow(duration_l, 1 - c);
                    double scale_r = pow(duration_r, 1 - c);
                    for (int d = 0; d < N_DIM; d++)
                    {
            ...
            // ~ start state constraints
            {
                ...
                double duration = cubes[0].t_ub - cubes[0].t_lb;
                double scale;
                int n = 0;
            ...
            // ~ end state constraints
            {
                ...
                double duration = cubes[num_segments - 1].t_ub - cubes[num_segments - 1].t_lb;
                double scale;
        ...
        for (int n = 0; n < num_segments; n++) {
            for (int j = 0; j < num_order; j++) {
                Eigen::Matrix<double, N_DIM, 1> coeff;
                for (int d = 0; d < N_DIM; d++)
    ...
    template <int N_DEG, int N_DIM> ErrorType SplineGenerator<N_DEG, N_DIM>::GetBezierSplineUsingCorridor(
        const vec_E<SpatioTemporalSemanticCubeNd<N_DIM>>& cubes,
        const vec_E<Eigen::Matrix<double, N_DIM, 1>>& start_constraints,
        const vec_E<Eigen::Matrix<double, N_DIM, 1>>& end_constraints,
        BezierSplineType* bezier_spline)
