...
namespace common
{
    ErrorType MobilLaneChangingModel::GetMobilAccChangesOnCurrentLane(
            ...
            const FrenetState &following_fs, double *acc_o, double *acc_o_tilda,
            double *acc_c) {
        bool has_leading_vehicle = leading_vehicle.id() != constants::kInvalidAgentId;
        bool has_following_vehicle = following_vehicle.id() != constants::kInvalidAgentId;
        double acc_o_tmp = 0.0, acc_o_tilda_tmp = 0.0, acc_c_tmp = 0.0;
        ...
        if ((!has_following_vehicle) || fabs(following_fs.vec_s(1)) < constants::kEPS) {
    ...
    ErrorType MobilLaneChangingModel::GetMobilAccChangesOnTargetLane(
            ...
            const FrenetState &leading_fs, const Vehicle &following_vehicle,
            const FrenetState &following_fs, bool *is_lc_safe, double *acc_n,
            double *acc_n_tilda, double *acc_c_tilda) {
        bool has_leading_vehicle   = leading_vehicle.id() != constants::kInvalidAgentId;
        bool has_following_vehicle = following_vehicle.id() != constants::kInvalidAgentId;

        double acc_n_tmp = 0.0, acc_n_tilda_tmp = 0.0, acc_c_tilda_tmp = 0.0;

        *is_lc_safe = false;
        // ~ Safety check
        if (leading_vehicle.id() == following_vehicle.id() &&
            leading_vehicle.id() != constants::kInvalidAgentId) {
            *is_lc_safe = false;
        } else {
