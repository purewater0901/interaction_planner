#include "mobil/mobil_behavior_prediction.h"

namespace common
{
    ErrorType MobilBehaviorPrediction::RemapGainsToProb(
            const bool is_lcl_safe, const double mobil_gain_left,
            const bool is_lcr_safe, const double mobil_gain_right,
        ...
        if (is_lcl_safe && is_lcr_safe) {
            double gain_l = normalize_with_bound(mobil_gain_left, lower_bound,
                                                    upper_bound, 0.0, 1.0);
            double gain_k =
                    normalize_with_bound(1.0, lower_bound, upper_bound, 0.0, 1.0);
            double gain_r = normalize_with_bound(mobil_gain_right, lower_bound,
                                                    upper_bound, 0.0, 1.0);

            double p_l = gain_l / (gain_k + gain_l + gain_r);
            double p_k = gain_k / (gain_k + gain_l + gain_r);
            double p_r = gain_r / (gain_k + gain_l + gain_r);
            ...
            double gain = normalize_with_bound(mobil_gain_left, lower_bound,
                                                  upper_bound, 0.0, 1.0);
            ...
            double gain = normalize_with_bound(mobil_gain_right, lower_bound,
                                                  upper_bound, 0.0, 1.0);
    ...
    ErrorType MobilBehaviorPrediction::LateralBehaviorPrediction(
        ...
        double acc_c = 0.0;
        double acc_o = 0.0, acc_o_tilda = 0.0;
        double politeness_coeff = 0.0;

        bool is_lcl_safe = false;
        bool is_lcr_safe = false;

        double mobil_gain_left = -constants::kInf;
        double mobil_gain_right = -constants::kInf;

        // * When the vehicle is almost stop, we assume the behavior is lane keeping
        double desired_vel = vehicle.state().velocity;
        if (fabs(desired_vel) < constants::kBigEPS) {
            res->SetEntry(common::LateralBehavior::kLaneChangeLeft, 0.0);
            res->SetEntry(common::LateralBehavior::kLaneChangeRight, 0.0);
            res->SetEntry(common::LateralBehavior::kLaneKeeping, 1.0);
            res->is_valid = true;
        ...
        if (lcl_lane.IsValid()) {
            ...
            if (is_lcl_safe) {
                double d_a_c = acc_c_tilda - acc_c;
                double d_a_n = acc_n_tilda - acc_n;
                double d_a_o = acc_o_tilda - acc_o;
