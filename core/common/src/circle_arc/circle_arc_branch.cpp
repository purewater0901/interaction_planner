#include "circle_arc/circle_arc_branch.h"

namespace common
{
    void CircleArcBranch::RetFinalStates(std::vector<Eigen::Matrix<double,3,1>> *p_states) const
    {
        for (const auto &arc : circle_arc_vec_)
            p_states->emplace_back(arc.final_state());
    }

    void CircleArcBranch::RetAllSampledStates(std::vector<Eigen::Matrix<double, 3, 1>> *p_states) const
    {
        for (const auto &arc : circle_arc_vec_)
        {
    ...
    void CircleArcBranch::CalculateCircleArcBranch()
    {
        ...
        {
            std::cerr << "[CircleArcBranch] ERROR - Size of vec are not equal" << std::endl;
            assert(false);
        }
