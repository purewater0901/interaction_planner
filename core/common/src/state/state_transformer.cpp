...
namespace common
{
                                                        ...
                                                        State* s) const
    {
        ...
        double curvature, curvature_derivative;
        if (lane_.GetCurvatureByArcLength(fs.vec_s[0], &curvature, &curvature_derivative) != kSuccess)
            return kWrongStatus;

        double one_minus_curd = 1 - curvature * fs.vec_ds[0];
        if (one_minus_curd < constants::kEPS)
        {
        ...
        Eigen::Matrix<double,LaneDim,1> lane_pos;
        if (lane_.GetPositionByArcLength(fs.vec_s[0], &lane_pos) != kSuccess)
            return kWrongStatus;

        Eigen::Matrix<double,LaneDim,1> vec_tangent;
        if (lane_.GetTangentVectorByArcLength(fs.vec_s[0], &vec_tangent) != kSuccess)
            return kWrongStatus;

        double lane_orientation = vec2d_to_angle(vec_tangent);

        Eigen::Matrix<double,LaneDim,1> vec_normal(-vec_tangent[1], vec_tangent[0]);
        double tan_delta_theta = fs.vec_ds[1] / one_minus_curd;
        double delta_theta = atan2(fs.vec_ds[1], one_minus_curd);
        ...
        double lhs = (fs.vec_ds[2] + (curvature_derivative * fs.vec_ds[0] + curvature * fs.vec_ds[1]) *
                      tan_delta_theta) * cn_delta_theta * cn_delta_theta / one_minus_curd;
        s->curvature = (lhs + curvature) * cn_delta_theta / one_minus_curd;
        double delta_theta_derivative = 1.0 /
                                                        ...
                                                        FrenetState* fs) const
    {
        ...
        Eigen::Matrix<double,2,1> lane_position;
        if (lane_.GetPositionByArcLength(arc_length, &lane_position) != kSuccess)
            return kWrongStatus;

        Eigen::Matrix<double,2,1> lane_tangent_vec;
        if (lane_.GetTangentVectorByArcLength(arc_length, &lane_tangent_vec) != kSuccess)
            return kWrongStatus;

        double lane_orientation = vec2d_to_angle(lane_tangent_vec);
        Eigen::Matrix<double,2,1> lane_normal_vec = Eigen::Matrix<double,2,1>(-lane_tangent_vec[1], lane_tangent_vec[0]);
        ...
        double d = (s.vec_position - lane_position).dot(lane_normal_vec);
        double one_minus_curd = 1 - curvature * d;
        if (one_minus_curd < constants::kEPS)
            return kWrongStatus;

        double delta_theta = normalize_angle(s.angle - lane_orientation);
        ...
        double sp = s.velocity * cn_delta_theta / one_minus_curd;

        double delta_theta_derivative =
        ...
        fs->Load(Eigen::Matrix<double,3,1>(arc_length, sp, spp), Eigen::Matrix<double,3,1>(d, ds, dss),
                 FrenetState::kInitWithDs);
        fs->time_stamp = s.time_stamp;
        return kSuccess;
    }

    ErrorType StateTransformer::GetFrenetPointFromPoint(const Eigen::Matrix<double,2,1>& s,
                                                        Eigen::Matrix<double,2,1>* fs) const
    {
        ...
        double d = (s - lane_position).dot(lane_normal_vec);
    ...
    ErrorType StateTransformer::GetFrenetPointVectorFromPoints(const vec_E<Eigen::Matrix<double,2,1>>& pts,
                                                               vec_E<Eigen::Matrix<double,2,1>>* fps) const
    {
        fps->clear();
        fps->reserve(pts.size());
        Eigen::Matrix<double,2,1> fp;
