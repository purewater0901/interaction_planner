 ...
 */
    ...
    public:
        ...
        void set_coeff(const int segment_idx, const int ctrl_pt_index,
                       const Eigen::Matrix<double,N_DIM,1>& coeff)
        {
            ctrl_pts_[segment_idx].row(ctrl_pt_index) = coeff;
        }

        void set_ctrl_pts(const vec_E<Eigen::Matrix<double, N_DEG + 1, N_DIM>>& pts)
        {
            ctrl_pts_ = pts;
        }
         ...
         */
        vec_E<Eigen::Matrix<double, N_DEG + 1, N_DIM>> ctrl_pts() const { return ctrl_pts_; }
         ...
         */
        ErrorType evaluate(const double s, const int d, Eigen::Matrix<double, N_DIM, 1>* ret) const
        {
            ...
            if (s < vec_domain_[0]) {
                ...
                Eigen::Matrix<double, N_DEG + 1, 1> basis = BezierUtils<N_DEG>::GetBezierBasis(d, normalized_s);
                Eigen::Matrix<double, N_DIM, 1> result = (pow(duration, 1 - d) * basis.transpose() * ctrl_pts_[idx]).transpose();
                *ret = result;
            }
            return kSuccess;
        }
    ...
    private:
        vec_E<Eigen::Matrix<double, N_DEG + 1, N_DIM>> ctrl_pts_;
