#ifndef SRC_POLYNOMIAL_H
#define SRC_POLYNOMIAL_H
...
namespace common
{
    ...
    */
    ...
    {
    public:
        typedef Eigen::Matrix<double, N_DEG + 1, 1> VecNf;
        ...
        inline double evaluate(const double& s) const
        {
            ...
            double p = coeff_normal_order_[N_DEG];
         ...
         * @param S, duration
         */
        void GetJerkOptimalConnection(const double p1, const double dp1,
                                      const double ddp1, const double p2,
                                      const double dp2, const double ddp2,
                                      const double S) {
            assert(N_DEG >= 5);
            ...
            Eigen::Matrix<double,6,6> A_inverse;
         ...
         */
        bool LookUpCache(const double S, Eigen::Matrix<double,6,6>* A_inverse)
        {
    ...
    public:
         ...
         */
        inline void evaluate(const double s, int d, Eigen::Matrix<double,N_DIM,1>* vec) const
        {
            for (int i = 0; i < N_DIM; i++)
                (*vec)[i] = polys_[i].evaluate(s, d);
        }

        inline void evaluate(const double s, Eigen::Matrix<double,N_DIM,1>* vec) const
        {
            for (int i = 0; i < N_DIM; i++)
